...
# Install (auto-detects port/pacman/apt/brew)
./install.sh

# Or manually stow individual packages
stow -t ~ alacritty fish tmux zellij starship git bash localbin nix claude agents

# Optional: private overlays (personal configs not kept in the public repo)
# Clone your private dotfiles repo alongside this one:
#   git clone <private-repo> ~/src/dotfiles-private
# Then add symlinks for any private packages you want to stow:
#   ln -sfn ../dotfiles-private/claude-private ~/src/dotfiles/claude-private
#   ln -sfn ../dotfiles-private/git-private ~/src/dotfiles/git-private
#   ln -sfn ../dotfiles-private/opencode-private ~/src/dotfiles/opencode-private
# Then re-run install.sh — it auto-detects and stows private overlays.
#+end_src
...
#+begin_src sh
sudo pacman -S --needed stow fish starship git-delta mise fzf fd \
    zoxide bat tmux git-lfs jq usage zellij
#+end_src
...
| agents    | ~.config/AGENTS.md~, shared ~.agents/skills/~ |
...
Shared cross-agent skills live under ~/.agents/skills/~ and are linked into Claude Code, Codex, and OpenCode by the installer. Claude-only skills still live under ~/.claude/skills/~.
...
- Machine-local shell settings live outside this repo:
  - ~~/.config/shell/local.sh~~ is sourced by bash and zsh if present
  - ~~/.config/fish/local.fish~~ is sourced by fish if present
...
Additional system packages beyond what ~install.sh~ handles:

#+begin_src sh
sudo pacman -S --needed git emacs ripgrep base-devel xclip cmake docker
#+end_src
